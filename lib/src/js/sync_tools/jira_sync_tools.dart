...
class JiraSyncTools {
  ...
  /// `jira_update_field` — the Java `updateField` engine.
  ///
  /// `""` clears the field (`{fields:{field:null}}`, Java `clearField`).
  /// Strings are coerced to bool/int/double/JSON. `customfield_*` ids and
  /// system fields PUT `{update:{field:[{set:v}]}}` directly; any other
  /// field name resolves to ALL active customfields with that name and
  /// updates each with a `✅`/`❌` per-field summary.
  String _updateField(Map<String, dynamic> args) => _run((config) {
        final key = _asStr(args['key']);
        final field = _asStr(args['field']);
        final value = args['value'];
        if (value is String && value.isEmpty) {
          return _putBody(config, '${config.baseUrl}/issue/$key',
              jsonEncode({
                'fields': {field: null},
              }));
        }
