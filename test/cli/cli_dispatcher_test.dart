...
/// gh-241: the engine-worker pool boots lazily inside the dispatcher —
/// only when the resolved job config enables `runAsync`
/// (`parallelWorkers >= 2`), just before the JS run. Every other command
/// (and every job without the knob) keeps zero pool cost, and a failed
/// boot degrades to the documented unbooted-pool JS error instead of
/// killing the command.
void _testRunAsyncPoolBoot() {
  group('run: lazy engine-worker pool boot (gh-241)', () {
    ...
    test('parallelWorkers >= 2 boots the pool before the JS run', () async {
      final script = File('${_tmp.path}/pool_probe.js')
        ..writeAsStringSync(
...
''');
    ...
    test('parallelWorkers < 2 leaves the pool unbooted', () async {
      final script = File('${_tmp.path}/pool_seq.js')
        ..writeAsStringSync(
          'function action(params) { return typeof runAsync; }',
        );
      final config = File('${_tmp.path}/pool_seq_job.json')
        ..writeAsStringSync(jsonEncode({
          'name': 'jsrunner',
          'params': {
            'jsPath': script.path,
            'jobParams': {'parallelWorkers': 1},
          },
        }));
      expect(await _dispatcher.dispatch(['run', config.path]), 0);
    ...
    test('a failed boot degrades to the unbooted-pool JS error', () async {
      ...
      final script = File('${_tmp.path}/pool_boom.js')..writeAsStringSync('''
function action(params) {
  try {
...
''');
      final config = File('${_tmp.path}/pool_boom_job.json')
        ..writeAsStringSync(jsonEncode({
          'name': 'jsrunner',
          'params': {
            'jsPath': script.path,
