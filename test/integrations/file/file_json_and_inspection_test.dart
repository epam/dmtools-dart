...
/// Tests for the JSON and inspection file tools (`file_read_json`,
/// `file_write_json`, `file_exists_in_path`, `file_get_size`) in
/// [FileToolExecutor] and [fileTools].
void main() {
  catalogTests();
  readJsonTests();
  writeJsonTests();
...
Directory _tempDir() =>
    Directory.systemTemp.createTempSync('dmtools_file_json_inspection_');

/// Catalog checks for the JSON and inspection tools.
void catalogTests() {
  final tools = fileTools();

  group('fileTools catalog: json/inspection tools', () {
    test('registers nineteen tools total', () {
      expect(tools, hasLength(19));
    });

    test('includes the json/inspection tool names', () {
    ...
    test('every json/inspection tool belongs to the file integration', () {
      for (final name in [
        'file_read_json',
        'file_write_json',
        'file_exists_in_path',
...
/// Dispatch tests for the JSON and inspection tools.
void dispatchTests() {
  group('execute dispatch: json/inspection tools', () {
    late FileToolExecutor executor;
    late Directory tempDir;
