...
void compositeKeyTests() {
  group('composite key resolution', () {
    compositeKeyReadTests();
    compositeKeyMutationTests();
    compositeKeyCreateIssueTests();
  });
}

/// `github_get_issue` key parsing (composite, bare, override, reject).
void compositeKeyReadTests() {
...
/// Mutation tools accept composite keys end to end.
void compositeKeyMutationTests() {
  ...
  test('github_add_labels accepts a composite key', () async {
    final f = mockGithub((o) => routeByPath({'/labels': '[]'}, o));
    await f.client.addLabels(null, null, null, ['bug'], key: 'myorg/myrepo#5');
    expect(
      f.adapter.calls.single.path,
  ...
  test('github_remove_label accepts a composite key', () async {
    ...
    await f.client.removeLabel(null, null, null, 'bug', key: 'myorg/myrepo#5');
    expect(
      f.adapter.calls.single.path,
  ...
  test('github_create_comment accepts a composite key', () async {
    ...
    await f.client
        .createComment(null, null, null, 'Looks good!', key: 'myorg/myrepo#5');
    final call = f.adapter.calls.single;
    expect(call.method, 'POST');
    expect(
      call.path,
...
/// `github_create_issue` project-key and default handling.
void compositeKeyCreateIssueTests() {
...
void trackerCatalogTests() {
  ...
  group('github tracker catalog', () {
    trackerCatalogRegistrationTests(registry);
    trackerCatalogAliasTests(registry);
    trackerCatalogParamTests(registry);
  });
}

/// Registration and Java alias sets of the tracker tools.
void trackerCatalogRegistrationTests(ToolRegistry registry) {
...
/// Per-tool parameter shapes of the new tracker tools.
void trackerCatalogParamTests(ToolRegistry registry) {
  ToolDefinition tool(String name) => registry.getTool(name)!;
...
/// Composite-key params on existing tools plus alias application.
void trackerCatalogAliasTests(ToolRegistry registry) {
  ToolDefinition tool(String name) => registry.getTool(name)!;
  ...
  test('applyParamAliases maps the tracker aliases onto canonical names', () {
    ...
    final assigned =
        tool('github_assign_issue').applyParamAliases({'assignee': 'octocat'});
...
void trackerExecutorTests() {
  group('GithubToolExecutor.execute (tracker tools)', () {
    trackerExecutorReadTests();
    trackerExecutorMutationTests();
  });
}

/// Executor routing of the read-side tracker tools.
void trackerExecutorReadTests() {
...
/// Executor routing of the write-side tracker tools.
void trackerExecutorMutationTests() {
